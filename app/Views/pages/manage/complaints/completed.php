...
<div class="space-y-6">
    <!-- Header -->
    <div>
        <h1 class="text-2xl md:text-3xl font-bold tracking-tight">Pengaduan Selesai</h1>
        <p class="text-muted-foreground mt-2">Arsip pengaduan yang sudah selesai ditangani</p>
    </div>
    ...
    <div class="bg-card border border-border rounded-3xl shadow-sm overflow-hidden">
        ...
        <div id="empty-state" class="hidden py-16 text-center">
            ...
            <h3 class="text-lg font-medium text-foreground mb-2">Belum Ada Pengaduan yang Diselesaikan</h3>
            <p class="text-sm text-muted-foreground max-w-sm mx-auto">Selesaikan pengaduan dari halaman Diproses untuk melihat hasilnya di sini.</p>
        </div>
...
<script>
    ...
    const viewLink = `<?= base_url('/manage/complaint/completed/detail/') ?>`;
        ...
        rowTemplate: (complaint, index) => {
            return `
                <tr>
                    ...
                    <td><span class="text-sm text-foreground">${escapeHtml(complaint.completed_by_name)}</span></td>
